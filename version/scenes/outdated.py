...
def outdated(logger, pygame, screen, clock, assets, options, lang, windowsize, newerversion):
    ...
    def wrappedtext(text, font, maxwidth):
        ...
        text = text[:-1]
        for i in text:
            r = font.render(i, False, (255, 170, 0))
            rsize = r.get_rect().size
    ...
    logger.debug(f'Loaded Scene "outdated"')

    logger.warning(f'There Is A Newer Version Of The Game ({newerversion}).')
    ...
    while not end:
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    for i in range(2):
                        if mousepos[0] >= windowsize[0] / 2 - buttonsize[0] - 10 + i % 2 * (buttonsize[0] + 10) and mousepos[0] <= windowsize[0] / 2 - 10 + i % 2 * (buttonsize[0] + 10) and mousepos[1] >= windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2) and mousepos[1] <= windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2) + buttonsize[1]:
                            try:
        ...
        # Error Message
        message = wrappedtext(getlang('outdated.message', lang).replace('{NEWVERSION}', newerversion), getfont('debug', assets), windowsize[0])
        ...
        for i in range(2):
            if mousepos[0] >= windowsize[0] / 2 - buttonsize[0] - 10 + i % 2 * (buttonsize[0] + 10) and mousepos[0] <= windowsize[0] / 2 - 10 + i % 2 * (buttonsize[0] + 10) and mousepos[1] >= windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2) and mousepos[1] <= windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2) + buttonsize[1]:
                display.blit(hbutton, (windowsize[0] / 2 - buttonsize[0] - 10 + i % 2 * (buttonsize[0] + 10), windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2)))
            else:
                display.blit(button, (windowsize[0] / 2 - buttonsize[0] - 10 + i % 2 * (buttonsize[0] + 10), windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2)))
            buttontext = colouredtext(splitcolours(getlang(f'outdated.buttons_{i}', lang)), getfont('font', assets))
            ...
            display.blit(buttontext, (windowsize[0] / 2 - buttonsize[0] / 2 - 10 + i % 2 * (buttonsize[0] + 10) - buttontextsize[0] / 2, windowsize[1] / 2 + buttonsize[1] * (5 + floor(i / 2)) + 10 * floor(i / 2) + buttontextsize[1] / 2))
    ...
    return((assets, options, lang, windowsize))