...
def unknown(logger, pygame, screen, clock, assets, options, lang, windowsize):
    ...
    logger.debug(f'Loaded Scene "unknown"')

    logger.error(f'Failed To Check Most Recent Version. This Version May Be Broken.')
    ...
    while not end:
        ...
        # Error Message
        message = wrappedtext(getlang('unknown.message', lang), getfont('debug', assets), windowsize[0])
        ...
        for i in range(2):
            ...
            buttontext = colouredtext(splitcolours(getlang(f'unknown.buttons_{i}', lang)), getfont('font', assets))
